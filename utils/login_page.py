...
def verify_password(stored_password, provided_password):
    # Using MD5 as per user requirements for axusers
    return stored_password == hash_password_md5(provided_password)
...
def login():
    ...
    if st.session_state.page == "login":
        ...
        with collog2.form("login_form"):
            ...
            if submitted:
                # Authenticate against Oracle DB 'axusers' table
                try:
                    auth_query = f" select username, password from axusers where username = '{username}' and username <> 'admin' union all select 'admin' username,'0192023a7bbd73250516f069df18b500' password from dual  where 'admin' = '{username}' "
                    user_df = sr.fetch_sql_data(auth_query)
                    
                    if not user_df.empty:
                        stored_md5 = user_df.iloc[0]['PASSWORD']
                        
                        if verify_password(stored_md5, password):
                            st.session_state.logged_in = True
                            st.session_state.username = username
                            st.session_state.role = "user" # Default role, will be updated by perms

                            # Cache Permissions from dashboard_access
                            try:
                                ...
                                if not perms_df.empty:
                                    st.session_state.user_perms = perms_df.iloc[0].to_dict()
                                    if st.session_state.user_perms.get('SUPERUSER') == 'T':
                                        st.session_state.role = "admin"
                            except Exception as e:
                                pass
